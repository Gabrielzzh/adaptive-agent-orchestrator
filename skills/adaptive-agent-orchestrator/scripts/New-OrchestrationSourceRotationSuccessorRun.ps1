...
if (Test-Path -LiteralPath $runRoot) {
    throw "Source-rotation successor run already exists: $runRoot"
}
...
try {
    ...
    $sourceBindings = @($export.rotated_source_bindings |
        ForEach-Object {
            [ordered]@{
    ...
    $payload = [ordered]@{
        schema_version = '1.2'
        lineage_kind = 'source-rotation-successor'
        run_path = $runRoot
        ...
        rotation_target_milestone_id =
            [string]$export.rotation_target_milestone_id
        rotation_checkpoint_material_path =
            [string]$export.rotation_checkpoint_material_path
        rotation_checkpoint_material_hash =
            [string]$export.rotation_checkpoint_material_hash
        ...
        inherited_obligations_hash = [string]$export.open_obligations_hash
    ...
    $event = [ordered]@{
        ...
        event = 'durable-review-source-rotation-adopted'
        run_id = [string]$run.run_id
        plan_hash = [string]$run.plan_hash
        ...
        status = 'planned'
        message = (
            "Adopted fresh reviewer seats from '$($export.predecessor_run_id)'."
        )
        thread_id = $null
        model_id = $null
        ...
        evidence = @(
            "artifact:receipts/$receiptName",
            "source:predecessor:$([string]$export.predecessor_run_id)"
        )
        result_receipt_path = "receipts/$receiptName"
        result_receipt_hash = [string]$receipt.receipt_hash
        idempotency_key = 'controller:source-rotation-successor-adoption'
