...
param(
    ...
    [Parameter(Mandatory)][string] $CumulativeCorrectionKey
)
...
if ($CumulativeCorrectionKey -cnotmatch
    '^(user|controller):[A-Za-z0-9][A-Za-z0-9._:-]{0,191}$') {
    throw 'Milestone revision cumulative correction requires a stable authority key.'
}
foreach ($candidate in @(
    ...
    if (-not $full.StartsWith(
        ...
        throw (
            'Milestone revision cumulative correction inputs must be files ' +
            'inside the run.'
        )
    }
}
if ([string]::IsNullOrWhiteSpace(
    ...
    throw 'Milestone revision cumulative correction authorization is empty.'
}
...
$cumulativeCorrectionEvents = @($events | Where-Object {
...
if ($authorizationEvents.Count -ne 1 -or $selectionEvents.Count -ne 0 -or
    $cumulativeCorrectionEvents.Count -ne 0) {
    throw (
        'Milestone revision cumulative correction requires one pending, ' +
        'unsuperseded authorization.'
    )
}
if ($correctionEvents.Count -ne 1 -or
    -not (Test-Path -LiteralPath $correctionReceiptPath -PathType Leaf)) {
    throw 'Cumulative correction requires one exact lifecycle correction.'
}
...
if ($selectionItems.Count -ne $requiredSources.Count) {
    throw 'Milestone revision cumulative correction requires every source.'
}
...
foreach ($requiredSource in $requiredSources) {
    ...
    if ($items.Count -ne 1) {
        throw "Cumulative correction source '$sourceNodeId' is missing or repeated."
    }
    ...
    if ([string]$currentBinding.checkpoint_material_path -ne
        ...
        throw "Cumulative correction source '$sourceNodeId' changed scope."
    }
    ...
    $rearms = @(Get-DurableReviewMilestoneRevisionRearmEvent `
    ...
    if ([string]$continuity.source_thread_id -ne
        [string]$currentBinding.source_thread_id) {
        throw (
            "Cumulative correction source '$sourceNodeId' cannot change " +
    ...
    $completed = @($sourceEvents | Where-Object {
        ...
        [string]$_.thread_id -eq [string]$continuity.source_thread_id -and
    ...
    if ($null -eq $completed -or $null -eq $validated -or $null -eq $adopted -or
        ...
        ($null -eq $lifecycleCorrection -and (
            ...
            "artifact:$($currentBinding.disposition_receipt_path)" -notin
                @($adopted.evidence)))) {
        throw "Cumulative correction source '$sourceNodeId' has invalid lifecycle."
    }
    if ($null -ne $lifecycleCorrection) {
        ...
        if ($sourceCorrection.Count -ne 1 -or
            [int]$sourceCorrection[0].rearm_event_sequence -ne [int]$rearms[0].sequence -or
            [string]$sourceCorrection[0].rearm_event_hash -ne [string]$rearms[0].hash -or
            ...
            throw "Cumulative correction source '$sourceNodeId' correction changed."
        }
    }
    $previousDeclared = @($authorization.previous_source_bindings |
        Where-Object { [string]$_.source_node_id -eq $sourceNodeId })
    if ($previousDeclared.Count -ne 1) {
        throw 'Cumulative correction predecessor binding is incomplete.'
    }
    ...
    foreach ($bindingName in @(
        'source_node_id', 'source_thread_id', 'milestone_id',
        'checkpoint_material_path', 'checkpoint_material_hash',
        'result_receipt_path', 'result_receipt_hash', 'result_file_hash',
        'disposition_receipt_path', 'disposition_receipt_hash',
        'disposition_file_hash'
    )) {
        if ([string]$previousBinding.$bindingName -cne
            [string]$previousDeclared[0].$bindingName) {
            throw "Cumulative correction source '$sourceNodeId' predecessor changed."
        }
    }
    ...
    $resultRelativePath = (
        ...
        'cumulative-corrected.thread-result-receipt.json'
    )
    $dispositionRelativePath = (
        "receipts/$sourceNodeId.revision-$revisionToken." +
        'cumulative-corrected.disposition.json'
    )
    ...
    if ((Test-Path -LiteralPath $resultPath) -or
        ...
        throw "Cumulative correction source '$sourceNodeId' output already exists."
    }
...
if ($restoredTotal -lt 1) {
    throw 'Milestone revision cumulative correction found no omitted occurrence.'
}
} finally {
...
$effectiveSelectionName = (
    "durable-review-milestone.$($authorization.milestone_id)." +
    "revision-$revisionToken.cumulative-corrected-selection.json"
)
...
if ((Test-Path -LiteralPath $effectiveSelectionPath) -or
    ...
    throw 'Milestone revision cumulative correction already exists.'
}
...
try {
    ...
    foreach ($artifactPlan in @($artifactPlans)) {
        $supersededBinding = [pscustomobject][ordered]@{
            ...
            source_thread_id = [string]$artifactPlan.continuity.source_thread_id
    ...
    $payload = [ordered]@{
        ...
        prior_selection_material_path = & $relative $SelectionMaterialPath
        prior_selection_material_hash = (
        ...
        cumulative_correction_sources = @($sourceSupersessions)
        cumulative_correction_sources_hash = Get-TextSha256 (
        ...
        cumulative_correction_key = $CumulativeCorrectionKey
        created_at_utc = [DateTimeOffset]::UtcNow.ToString('o')
    }
    if ($null -ne $lifecycleCorrection) {
        $payload.lifecycle_correction_receipt_path =
            "receipts/$($correctionReceiptPath | Split-Path -Leaf)"
    ...
    try {
        ...
        if ($current.Count -ne $events.Count -or
            ...
            throw 'The journal changed while cumulative correction was prepared.'
        }
        ...
        $event = New-MilestoneRevisionJournalEvent -Plan $plan -Run $run `
                ...
                -EventName 'milestone-revision-cumulative-corrected' `
                ...
                ) -IdempotencyKey $CumulativeCorrectionKey
...
$verified = Read-DurableReviewMilestoneRevisionCumulativeCorrection `
