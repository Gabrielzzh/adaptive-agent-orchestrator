...
运营任务的 Codex Skill。调用这个 Skill 的当前 Agent 是本任务负责人：先
理解目标、拆分独立工作流，再决定所需的层级、角色和模型。简单任务可一层
执行；复杂任务可形成“项目负责人→执行者”；只有多个项目才形成多个项目
负责人。
...
- **限制上下文选择：** 项目根目录、全部文件等宽泛占位引用会被拒绝；可选
  的选择理由只留在任务负责人控制面，不进入 Worker prompt。
- **渐进披露：** Skill 正文保持精简；reference 和项目文件只在当前工作流
  真正需要时读取。
- **默认单层执行：** 小任务、强顺序、高上下文重叠和窄范围修改留在当前
  任务负责人这一层。
- **值得时使用长期任务：** 独立、有边界、可验收，且能使用更小上下文或
  更低成本模型的工作会主动建议创建可见的 Codex 长期任务，而不是静默留在
  昂贵的主上下文中。
- **动态公司式组织：** 当前任务负责人只建立任务实际需要的层级；项目负责
  人协调自己的项目，执行者负责有边界的工作流；只有多个项目才设置多个
  项目负责人。
- **向上增员边界：** 非项目负责人的执行者不能递归招人；需要更多人力或
  容量时，只能向直接上级提出有边界的申请，由上级决定是否增员以及归属。
- **Writer 隔离与清理：** 每个 Writer 使用独立 worktree。完成后由任务或
  项目负责人核验并采纳结果，归档已完成任务，清理 worktree 和临时产物。
...
- **跨里程碑审核推进：** 同一个长期审核 run 可通过追加式收据激活下一条已
  声明里程碑，精确绑定来源链并要求负责人重新验收；无需改写 plan，也不
  会按文件时间猜测“最新结果”。
...
- **不可信结果边界：** 任务负责人的控制面政策把 Worker 输出视为不可信数据，
...
- **成本优先的模型路由：** 普通执行优先 Luna High/Max；复杂管理、复杂判断、
  架构和正式审核使用 Sol High/Max。Terra 不是默认选项，必须有用户明确
  请求或授权；Ultra 必须逐节点明确批准。运行时不公开实际模型时，请求模型
  与实际模型保持分离，并将实际模型记为 unverified。
...
- **通用产物所有权：** 任务负责人负责任务级整合，项目负责人负责项目级
  整合；执行者可拥有边界明确的章节、模块、调查、数据集或设计面。
...
- **向上增员控制：** 非项目负责人的执行者只能向直接上级申请增员，不能
  自行递归创建无限组织。
...
| 完成判断 | 任务/项目负责人整合 | 节点、产物、证据和人工门检查 |
...
请求
  ↓
当前 Agent 负责本任务，理解目标并选择单层或“项目负责人→执行者”层级
  ↓
寻找独立工作流并分配有边界的所有权
  ↓
只启动能够增加已接受价值的 Worker
  ↓
任务/项目负责人验证并采纳结果
  ↓
由负责人归档并清理 Writer 的 worktree
  ↓
按风险审阅 + 负责人整合
  ↓
产物、证据和人工决策完成门
...
脚本负责结构和生命周期状态校验。任务负责人和项目负责人负责选择可用执行
工具、创建 Worker、读取真实线程、整合结果，并在各自范围内执行已授权的外部
动作。
...
- 真实 37-event run 通过预授权进入计划内 Group2：11 条 P1 中 4 条经同
  来源复审解决、7 条继续保留；completion 仍因这 7 条问题和缺少负责人最终
...
- 真实 fresh-review run 通过受限相邻物化路径接回一个已经创建的任务，只再
  ...
  缺少负责人最终验收而 `BLOCKED`，没有发现新的 Orchestrator P0/P1。
- 真实 checkpoint15 revision 对两条误绑的 `validated` 事件追加了一份完整
  ...
  6 条开放 P1 和缺少负责人验收而 `BLOCKED`，已解决的 P1 没有回流。
- 真实 checkpoint16 在前一轮已经完整选定、但仍保留开放问题且没有最终主
  ...
  负责人最终验收而 `BLOCKED`，本轮同来源解决的两项问题没有回流。
...
- 自然语言排除项无法删除宿主已经注入的历史；应使用 fresh Worker 和明确
  输入引用。
- 精确重叠检查无法发现“不同名称但语义相同”的材料，任务负责人仍需拒绝。
- 不同项目之间没有共享机器级校准账本；需要更多容量时由项目负责人向任务
  负责人申请，恢复后由负责人先核对可见状态再继续创建 Worker。
...
支付、账号修改和生产操作仍由任务负责人或明确指定的负责人持有，并需要
用户授权。
