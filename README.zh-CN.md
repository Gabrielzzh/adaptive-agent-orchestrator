...
运营任务的 Codex Skill。每次调用时，当前 Agent 成为该任务的公司式负责人：
先理解目标，再按需要形成工作流、层级和角色，完成调度、验收、整合与收尾。
简单任务保持一层；复杂任务或多项目任务才增加项目负责人和独立执行工作流。
...
- **渐进披露：** Skill 正文保持精简；reference 和项目文件只在当前工作流
  真正需要时读取。
- **按任务生成组织：** 小任务、强顺序、高上下文重叠和窄范围修改保持一层；
  只有任务确实需要时才形成负责人到执行者的更深结构。
...
- **成本优先的静态模型路由：** Luna 处理中等推理的机械任务，并以最大推理
  承担边界明确的普通实现与测试；Sol 负责架构、模糊调试以及正式领域/反方
  审查；Terra 只在用户明确指定时使用，正式任务前不会额外启动模型
  测试 Agent。平台不公开实际模型时，请求模型与实际模型保持分离，实际模型
  明确记为 unverified。
  很难但边界清楚的任务优先使用 Sol `max`；Ultra 还会启用自动委派，因此只作
  逐节点明确授权的最后手段。
...
- **向上申请增员：** 执行者需要新角色时向上级负责人申请，不能递归创建
  Worker 或未经批准扩张组织。
